...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/08 07:10:15 by hsuner            #+#    #+#             */
/*   Updated: 2023/11/02 16:23:58 by hsuner           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_qs_first_seperation(t_stack *stack, int len)
{
	...
	items = len;
	ft_get_median(&pivot_a, stack->a, len);
	while (len != items / 2 + items % 2)
	{
		...
		{
			ft_get_median(&pivot_b, stack->b, stack->size_b);
...
int	ft_quicksort_a(t_stack *stack, int len, int count)
{
	...
	if (ft_is_sorted(stack->a, len, 0) == 1)
		return (1);
	if (len == 2)
		return (ft_sort_small_a2(stack));
	else if (len == 3 && stack->size_a == 3)
		return (ft_sort_three(stack));
	else if (len == 3)
		return (ft_sort_small_a3(stack, len));
	ft_get_median(&pivot_a, stack->a, len);
	items = len;
	while (len != items / 2 + items % 2)
	{
...
int	ft_quicksort_b(t_stack *stack, int len, int count)
{
	...
	ft_get_median(&pivot_b, stack->b, len);
	items = len;
	while (len != items / 2)
	{
